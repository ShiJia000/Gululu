...
class availableFriend extends api {
	...
	public function doExecute() {
		...
		$sql = 'SELECT u.uid, u.firstname, u.lastname, b.bname FROM user u, join_block jb, block b,(SELECT jb.uid, hid FROM join_block jb, block b WHERE jb.uid=' . $uid . ' AND jb.bid=b.bid AND jb.is_approved=1) as t WHERE u.uid = jb.uid AND jb.bid=b.bid AND b.hid = t.hid AND is_approved=1 AND u.uid<>' . $uid . ' AND u.uid NOT IN (SELECT friend_uid FROM friend WHERE uid=' . $uid . ' AND (is_valid=1 OR is_valid=0));';
